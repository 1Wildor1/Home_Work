...
namespace Home_Work.DAL.Repositories
{
    public class StudentRepository : IRepositoryInterface
    {
        ...
        public void UpDate(int id, string name, string lastName, string YearOfStudy)
        {
            using (var sqlConnection = new SqlConnection(connection_string))
            {
                sqlConnection.Open();

                using (var cmd = new SqlCommand(StudentSqlCreator.UpDate(id, name, lastName, YearOfStudy).SqlText, sqlConnection))
                {
